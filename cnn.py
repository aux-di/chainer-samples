...
class MLP(chainer.Chain):
    ...
    # input -> conv1 -> pooling -> layer2

    def __init__(self, n_in, n_units, n_out):
        super(MLP, self).__init__(
            conv1=F.Convolution2D(1, 4, 3, pad=0),
            l1=L.Linear(3600, n_units),  # first layer
            l2=L.Linear(n_units, n_out),  # output layer
        )
...
def main():
    ...
    for file, label in zip(csv['file'], csv['label']):
        ...
        imgdata = np.array(gray, dtype='f').reshape(1, 32, 32) / 255.0
